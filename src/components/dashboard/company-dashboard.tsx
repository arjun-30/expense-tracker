...
/** Company-wide analytics dashboard — SUPER_ADMIN/ADMIN/ACCOUNTS only, enforced
 * both by the caller (dashboard/page.tsx checks canViewCompanyDashboard before
 * rendering this) and inside every one of these service functions themselves. */
export async function CompanyDashboard({ session }: { session: SessionPayload }) {
  const [
    kpis,
    trend,
    trendYoy,
